...
async def time_bot(ctx):
    ...
    embed.add_field(name = 'Текущее время и дата в Москве (Россия :flag_ru:):', value = nowtime1, inline = False)
    embed.add_field(name = 'Текущее время и дата в Нью-Йорке (Америка :flag_us:):', value = nowtime2, inline = False)
    embed.add_field(name = 'Текущее время и дата в Токио (Япония :flag_jp:):', value = nowtime3, inline = False)
    embed.set_footer(text = "supports by quantprod")
    await ctx.send(embed = embed)
