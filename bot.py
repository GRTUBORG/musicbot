...
async def on_member_join(member):
    ...
    embed.add_field(name = "Приветственное сообщение", value = "Добро пожаловать в Dark Neon City!\n"
                                                               '\n'
                                                               "Перед тем, как пользоваться сервером, прочитай `#правила`. Это обязательно, а то атата!)\n"
                                                               '\n'
                                                               ...
                                                               '\n'
                                                               "Chill'овая беседка - самое уютное место для общения на различные темы! Заглядывай туда, в `#смскер`, или же в `Беседку` под ним!\n"
                                                               '\n'
                                                               "На сервере, как ты заметил, присутствуют боты, информацию о них можешь найти в `#quantupdates`.\n"
                                                               '\n'
...
async def time_bot(ctx):
    ...
    embed.add_field(name = 'Текущее время и дата в Москве:', value = nowtime1, inline = False)
    embed.add_field(name = 'Текущее время и дата в Нью-Йорке:', value = nowtime2, inline = False)
    embed.add_field(name = 'Текущее время и дата в Японии:', value = nowtime3, inline = False)
    embed.set_footer(text = "supports by quantprod")
    await ctx.send(embed = embed)
